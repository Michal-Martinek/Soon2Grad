...

{
    ...
    class Program
    {
        ...
        {
            int ParseInt(string msg, bool overZero) {
                while (true) {
                    Console.Write(msg);
                    ...
                    if (overZero && result <= 0) {
                        Console.WriteLine("jmenovatel musí být > 0");
                        continue;
                    }
                    return result;
                }
            }
            // zavedl jsem si funkci která čte číslo a kontroluje správnost
            // opakuje dotaz dokud nejsou všechny požadavky splněny, poté jednoduše vrátí výsledek
            int a = ParseInt("Zadejte čitatel celé číslo: ", false);
            int b = ParseInt("Zadejte jmenovatel celé číslo větší než 0: ", true);
